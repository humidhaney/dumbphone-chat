...
def init_db():
    with closing(sqlite3.connect(DB_PATH)) as conn:
        ...
        # NEW: Table for ClickSend contact list sync tracking
        c.execute("""
...
def save_message(phone, role, content, intent_type=None, response_time_ms=None):
    ...
    # Store conversation context for key topics
    if role == "assistant" and content:
        # Extract and store key topics from assistant responses
...
# === NEW: ClickSend Contact List Management Functions ===
def create_clicksend_contact_list(list_name="Hey Alex Subscribers"):
    """Create a new contact list in ClickSend"""
    if not CLICKSEND_USERNAME or not CLICKSEND_API_KEY:
        return {"error": "ClickSend credentials not configured"}
    ...
    payload = {
        "list_name": list_name,
        "list_email_address": os.getenv("ADMIN_EMAIL", "admin@example.com")  # Required by ClickSend
    }
...
def get_clicksend_contact_lists():
    """Get all contact lists from ClickSend"""
    if not CLICKSEND_USERNAME or not CLICKSEND_API_KEY:
        return {"error": "ClickSend credentials not configured"}
...
def sync_whitelist_to_clicksend(list_id=None, list_name="Hey Alex Subscribers"):
    """Sync your whitelist to a ClickSend contact list"""
    ...
    # Prepare contacts for ClickSend
    contacts = []
    for i, phone in enumerate(whitelist):
        # Get usage stats for this contact
        stats = get_monthly_usage_stats(phone)
        
        contact = {
            "phone_number": phone,
            "first_name": f"User{i+1}",  # ClickSend requires a name
            "last_name": "",
            "email": "",  # Optional
            "custom_1": str(stats["current_count"]),  # Current usage
    ...
    # ClickSend accepts up to 1000 contacts per request
    batch_size = 1000
    results = []
...
def broadcast_via_clicksend_list(list_id, message):
    """Send a broadcast using ClickSend's contact list feature"""
    if not CLICKSEND_USERNAME or not CLICKSEND_API_KEY:
        return {"error": "ClickSend credentials not configured"}
...
# === ENHANCED Intent Detectors v2.3 ===
...
def detect_follow_up_intent(text: str, phone: str) -> Optional[IntentResult]:
    ...
    follow_up_patterns = [
        ...
        r'\bcontinue\b',
        r'\bgo\s+on\b',
        r'\band\?\s*$',  # Fixed: Catch "And?" questions
        r'\bsteps?\b',   # Catch recipe/tutorial requests
        r'\bfull\b.*\b(recipe|instructions)\b',  # Full recipe requests
        # NEW: Enhanced follow-up patterns
        r'\bdo\s+they\b',  # "Do they turn you into..."
        r'\bcan\s+they\b',  # "Can they cause..."
        r'\bwill\s+they\b', # "Will they affect..."
        r'\bare\s+they\b',  # "Are they dangerous..."
        r'\bwhy\s+do\s+they\b', # "Why do they..."
        r'\bhow\s+do\s+they\b'  # "How do they..."
    ]
    ...
    if any(re.search(pattern, text, re.I) for pattern in follow_up_patterns):
        ...
        # NEW: Check recent conversation history for context
        history = load_history(phone, limit=6)
        if history:
            # Look at the last few assistant messages for context
            recent_topics = []
            for msg in reversed(history[-4:]):  # Last 4 messages
                if msg["role"] == "assistant":
                    content = msg["content"].lower()
                    # Extract potential topics from recent responses
                    topic_keywords = [
            ...
            if recent_topics:
                context_topic = recent_topics[0]  # Use most recent topic
...
# UPDATED detector order v2.3 - cultural queries before restaurant
DET_ORDER = [
    detect_hours_intent,
